...
const BASE_INFOS = {
	...
	'speed': 200,#quanto maior a speed, mais rápido
...
var SHIP_UPGRADES = {
	...
	'speed': [1, 10, 4, "res://Images/UPGRADES/Icon_speed.png"],
...
var ENEMIES_INFOS = {#[nome do tipo, DMG, shoot delay, bulletspeed, speed, LIFE, projectile dies?, 'scene', 'bullet_scene' ...]
	'Enemy':        ['Enemy', 1, 3.5, 400, 50, 10, true, "res://Scenes/Enemies/Enemy.tscn", "res://Scenes/Enemies_projectiles/EnemyProjectile.tscn"],
	'BIG':          ['BIG', 4, 3, 350, 40, 40, true, "res://Scenes/Enemies/BIG.tscn", "res://Scenes/Enemies_projectiles/BIGProjectile.tscn"],
	'Double_shoot': ['Double_shoot', 2, 4, 380, 42, 30, true, "res://Scenes/Enemies/Double_shoot.tscn", "res://Scenes/Enemies_projectiles/EnemyProjectile.tscn"],
	'Fast_bullet':  ['Fast_bullet', 2, 4, 525, 35, 35, true, "res://Scenes/Enemies/Fast_bullet.tscn", "res://Scenes/Enemies_projectiles/EnemyProjectile.tscn"],
	'High_fire_rate':['High_fire_rate', 1, 1, 390, 35, 20, true, "res://Scenes/Enemies/High_fire_rate.tscn", "res://Scenes/Enemies_projectiles/EnemyProjectile.tscn"],
	'Mini':         ['Mini', 2, 2, 380, 80, 15, true, "res://Scenes/Enemies/Mini.tscn", "res://Scenes/Enemies_projectiles/EnemyProjectile.tscn"],
	'Arrow':        ['Arrow', 5, 3, 380, 48, 60, true, "res://Scenes/Enemies/Arrow.tscn", "res://Scenes/Enemies_projectiles/ArrowEnemyProjectile.tscn"],
	'Healthy':        ['Healthy', 4, 2.5, 400, 45, 180, true, "res://Scenes/Enemies/Healthy.tscn", "res://Scenes/Enemies_projectiles/EnemyProjectile.tscn"],
	'Wide':        ['Wide', 5, 3, 200, 30, 200, true, "res://Scenes/Enemies/Wide.tscn", "res://Scenes/Enemies_projectiles/WideProjectile.tscn"],
}
...
func play_sound(sound):
	match sound:
		'hit':
			hits.stop()
			hits.set_stream(preload("res://Mp3/Sounds/Hit.wav"))
			hits.play()
		'shoot':
			sounds.stop()
			sounds.set_stream(preload("res://Mp3/Sounds/Shoot.wav"))
			sounds.play()
		'startgame':
			sounds.stop()
			sounds.set_stream(preload("res://Mp3/Sounds/StartGame.mp3"))
			sounds.play()
...
func ADD_ENEMY(type = null, stronger = false):
	...
	if y == 1:
		...
		y = 1
	Global.RANDOM.randomize()
	var POS = Vector2((Global.RANDOM.randf_range(0, 1)*500+300)*x+get_tree().get_nodes_in_group("PLAYER")[0].global_position[1], (Global.RANDOM.randf_range(0, 1)*500+300)*y+get_tree().get_nodes_in_group("PLAYER")[0].global_position[1])
	for z in get_tree().get_nodes_in_group("ENEMY"):
		while POS.distance_to(z.global_position) < 70:
			POS = Vector2((Global.RANDOM.randf_range(0, 1)*500+300)*x+get_tree().get_nodes_in_group("PLAYER")[0].global_position[1], (Global.RANDOM.randf_range(0, 1)*500+300)*y+get_tree().get_nodes_in_group("PLAYER")[0].global_position[1])
	node.global_position = POS
	

func ADD_DMG_LABEL(pos, value, player = false):
	if len(DMG_LABEL_POOL) == 0:
		var node = preload("res://Scenes/Modules/Hit_info.tscn").instance()
		node.global_position = pos
		node.get_child(0).text = "%.1f" % value
		if player == true:
		...
		DMG_LABEL_POOL[0].global_position = pos
		DMG_LABEL_POOL[0].get_child(0).text = "%.1f" % value
		if player == true:
